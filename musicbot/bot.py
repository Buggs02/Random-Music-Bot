...
class MusicBot(discord.Client):
            ...
            for server in self.servers:
                for channel in server.channels:
                    for m in channel.voice_members:
                        if m.id in self.config.owner_id:
                            return m
        else:
    ...
    async def on_ready(self):
        ...
        if self.user.id in self.config.owner_id:
            raise exceptions.HelpfulError(
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```"
            helpmsg += "https://discord.gg/bmE6jty/ https://discord.gg/J5HpJD8"
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        for user in user_mentions.copy():
            if user.id in self.config.owner_id:
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        if self.user.bot:
            url = await self.generate_invite_link (permissions = discord.Permissions(administrator=True,  change_nickname=True,  use_voice_activation=True,  connect=True, speak=True,  create_instant_invite=True))
            return Response(
                "Please Use This Link To Invite Me To Your Server!  Click here: \n{}".format(url),
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        search_msg = await self.send_message(channel, "Searching for videos One Moment...")
        await self.send_typing(channel)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if author.id in self.config.owner_id \
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        delete_invokes = True
        delete_all = channel.permissions_for(author).manage_messages or author.id in self.config.owner_id
    ...
    async def cmd_disconnect(self, channel):
    ...
    async def on_message(self, message):
        ...
        if message.channel.is_private:
            if not (message.author.id in self.config.owner_id and command == 'joinserver'):
                await self.send_message(message.channel, 'You cannot use this bot in private messages.')
                return

        if message.author.id in self.blacklist and not message.author.id in self.config.owner_id:
        ...
        try:
            ...
            if not message.author.id in self.config.owner_id:
