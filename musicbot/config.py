...
class Config:
    def __init__(self, config_file):
        ...
        if not config.read(config_file, encoding='utf-8'):
            ...
            try:
                ...
                if not str(c.get('Permissions', 'OwnerID', fallback=0)): # jake pls no flame
        ...
        self.owner_id = str(config.get('Permissions', 'OwnerID', fallback=ConfigDefaults.owner_id))
    ...
    def run_checks(self):
        ...
        if self.owner_id and str(self.owner_id):
            if not str(self.owner_id):
                raise HelpfulError(
                    "OwnerID was not set.",
